...
For customers already in your database (step two), sending only the email address is enough to register the order to the shopper’s existing account.
...
For customers already in your data base (step two), it is enough to send this object only with an `addressId`, which you may get from step two.
...
The `selectedSla` field indicates the selected delivery option. You can choose an `id` value, from among the options in the `slas` array obtained in the cart simulation (step one). The corresponding `price` can also be found in the simulation response data, in the same object.
